...
| SH-20260714-001 | 已执行 | 用户确认 | V9 笔记系统 - 统一存储+浮窗+列表页 | 1）创建 NoteService.js：合并三个碎片化存储系统（exam/card/source），统一 localStorage 键 sh_v9_unified_notes，提供 getNotesByCard/getNotesByCardFlat 等接口，自动迁移旧数据；2）创建 NoteEditor.js：Markdown 编辑/预览、6 快选标签、自定义标签、ESC 关闭、Toast 反馈；3）创建 NoteListView.js：全部笔记列表、搜索、类型筛选、空状态；4）修复 LearnView.js bug：getNotesByCard 返回对象分组，buildNoteSection 误用 .find() 导致崩溃，改用 getNotesByCardFlat 返回数组；5）三入口接入：学习页（buildNoteSection）、顶部栏「笔记」按钮（showNoteList）、条文面板（loadMyNote/saveMyNote）；6）CSS 样式追加到 theme.css。验证：Vite 构建 0 错误，冒烟测试 11 项全部通过。 | app/v9/src/services/NoteService.js, app/v9/src/components/NoteEditor.js, app/v9/src/components/NoteListView.js, app/v9/src/components/LearnView.js, app/v9/src/app.js, app/v9/src/components/SourcePanel.js, app/v9/src/styles/theme.css | AI |
