...
export async function loadRecords() {
  ...
  try {
    if (await _isLoggedIn()) {
      const { loadClinicalRecords } = await import('@/api.js');
      const apiRecords = await loadClinicalRecords();
      if (apiRecords && apiRecords.length > 0) {
        // 缓存到本地
        _saveLocalRecords(apiRecords);
        return apiRecords;
      }
    }
  } catch (e) {
    console.warn('[ClinicalService] API 加载档案失败，回退本地:', e.message);
  }
  // 回退：本地 localStorage
  try {
    return JSON.parse(localStorage.getItem(CL_KEY) || '[]');
  } catch (e) {
    console.warn('[ClinicalService] 本地加载档案失败:', e);
    return [];
  }
}
...
function _saveLocalRecords(records) {
  try {
    ...
    console.warn('[ClinicalService] 本地保存档案失败:', e);
  }
}
...
/** 异步双写：创建记录到云端 */
async function _syncCreateRecord(record) {
  try {
    if (await _isLoggedIn()) {
      const { createClinicalRecord } = await import('@/api.js');
      await createClinicalRecord(record);
    }
  } catch (e) {
    console.warn('[ClinicalService] 云端同步创建档案失败:', e.message);
  }
}
...
export function updateRecord(id, updates) {
  ...
  saveRecords(records);
  // 异步云端同步
  _syncUpdateRecord(id, records[idx]);
  return true;
}

/** 异步双写：更新记录到云端 */
async function _syncUpdateRecord(id, record) {
  try {
    if (await _isLoggedIn()) {
      const { updateClinicalRecord } = await import('@/api.js');
      await updateClinicalRecord(id, record);
    }
  } catch (e) {
    console.warn('[ClinicalService] 云端同步更新档案失败:', e.message);
  }
}
...
/** 异步双写：从云端删除记录 */
async function _syncDeleteRecord(id) {
  try {
    if (await _isLoggedIn()) {
      const { deleteClinicalRecord } = await import('@/api.js');
      await deleteClinicalRecord(id);
    }
  } catch (e) {
    console.warn('[ClinicalService] 云端同步删除档案失败:', e.message);
  }
}
...
export function getRecordsByPerson(personId) {
  return loadRecordsLocal().filter(r => r.relatedPersonId === personId);
}
