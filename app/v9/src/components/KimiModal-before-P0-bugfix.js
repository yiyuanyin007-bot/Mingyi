...
export function buildTutorPrompt(card) {
  ...
  if (refs.teacher_notes && refs.teacher_notes.length > 0) {
    teacherNotesSection = '\n**📖 名家言论参考（请结合以下已有经验，在本回复中回应或拓展）：**\n' +
      refs.teacher_notes.slice(0, 4).map((n, i) =>
        `  ${i+1}. [${n.author || '佚名'}] ${n.summary ? n.summary.substring(0, 200) : n.title}`
      ).join('\n');
  }
  let clinicalCasesSection = '';
  if (refs.clinical_cases && refs.clinical_cases.length > 0) {
    clinicalCasesSection = '\n**🏥 已有临床医案（请参考以下案例，在第四部分鉴别自测中融入真实场景）：**\n' +
      refs.clinical_cases.slice(0, 3).map((c, i) => {
        const info = c.patient || c.diagnosis || '';
        const outcome = c.outcome ? ` → ${c.outcome.substring(0, 60)}` : '';
        return `  ${i+1}. ${c.title || '医案'}${info ? `（${info}）` : ''}${outcome}`;
      }).join('\n');
  }
...
- 病机：${c.pathology || '暂无'}
- 煎服法：${c.usage || '暂无'}
- 禁忌：${(c.contraindications || []).join('、') || '无'}${teacherNotesSection}${clinicalCasesSection}
...
- **「名家言论参考」和「临床医案」是你回答时的参考素材，不要逐条复述，而是将其融入你的分析之中：如名家观点可在第一部分概述或第二部分配伍中呼应，医案可在第四部分自测题中作为真实场景示例引用。**`;
}
